...
#else
// PreEmpt Priority field is [7:4]
#define PREEMPT_PRIORITY_FIELD          (3)
#endif
...
/****************************************************************************/
#if (defined JENNIC_CHIP_JN5179_MRA1)
// Jennic 12->0 range mapped onto ARM 0->7 range, priority tense is reversed
PUBLIC uint8 au8JENNICtoARMinterruptPriorityMapping[] =
{
        7, 7, 7, 7, 7, 7, 6, 5, 4, 3, 2, 1, 0
};

// ARM 0->7 range mapped onto Jennic 12->0 range, priority tense is reversed
PUBLIC uint8 au8ARMtoJENNICinterruptPriorityMapping[] =
{
        12, 11, 10, 9, 8, 7, 6, 5
};
#endif
        ...
        uint32                  u32EnableMask)
{
    ...
    // Store old priority level, and set new one if appropriate
    // store the actual register value here
    psIntStorage->u8Level = MICRO_GET_ACTIVE_INT_LEVEL();

    // use macros to read and write the priority values from/to the NVIC
    if(MICRO_ARM_TO_JENNIC_PRIORITY_MAP(MICRO_INTERRUPT_READ_PRIORITY_VALUE(psIntStorage->u8Level)) < 11)
    {
        MICRO_SET_ACTIVE_INT_LEVEL(
        MICRO_INTERRUPT_WRITE_PRIORITY_VALUE(MICRO_JENNIC_TO_ARM_PRIORITY_MAP(11)));
    }
